...
impl TheGraphResponseData {
    fn into_query_response(self) -> QueryResponse {
        ...
        for transaction in self.data.transactions {
            swaps.extend(transaction.swaps);
        }
        QueryResponse { swaps }
    }
}
...
pub fn query_swaps(block_number: u64, state: &State<MyState>) -> Result<String, NotFound<String>> {
    let the_graph_query = format!("
{{\"query\":\"  {{transactions(orderBy: blockNumber, orderDirection: desc, where: {{blockNumber: {}}}) {{swaps {{id}}}}}}\"}}", block_number);
